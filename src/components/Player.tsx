...
const Player: React.FC<PlayerProps> = ({
  ...
  const Particles = () => {
    ...
    return <>
        {[...Array(particleCount)].map((_, i) => <motion.div key={i} initial={{
        x: 0,
        y: 0,
        opacity: 0.8,
  ...
  return <div className="absolute will-change-transform hardware-accelerated" style={{
    left: player.x,
    top: player.y,
    ...
    transition: 'filter 0.3s ease'
      ...
      <AnimatePresence>
        ...
        height: player.height
          ...
          {/* Player eyes */}
          <div className="absolute flex justify-center space-x-3 pointer-events-none" style={{
          ...
          duration: 0.5
        }} className="absolute inset-0 rounded-md pointer-events-none" style={{
          ...
          {(player.currentElement === 'fire' || player.currentElement === 'air') && Math.abs(player.velocityX) > 1 && <motion.div className="absolute -z-10 h-full w-full opacity-50" animate={{
          opacity: [0.7, 0, 0],
          ...
          {player.currentElement === 'water' && (player.isMovingLeft || player.isMovingRight) && <motion.div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-12 h-2 rounded-full" animate={{
          scaleX: [1, 1.5, 0.8],
          opacity: [0, 0.5, 0]
        }} transition={{
          duration: 0.8,
          ...
          {player.currentElement === 'earth' && !player.isJumping && player.velocityY > 10 && <motion.div className="absolute -bottom-5 left-1/2 -translate-x-1/2 w-16 h-2" initial={{
          opacity: 0,
          scale: 0.5
        }} animate={{
          opacity: [0, 0.8, 0],
          ...
          {/* Ducking effect */}
          {player.isDucking && <motion.div className="absolute bottom-0 left-0 right-0 h-1.5 opacity-50" initial={{
          opacity: 0,
          scaleX: 0.5
        }} animate={{
          opacity: 0.5,
